---
title: "DATA698 Project - Midterm submission"
...
This project aims to explore and analyze the NYPD Arrests data to identify trends and patterns about arrests in New York City. The dataset to be use in this research is the NYPD Arrest Data and its gotten from the NYC Open data (NYPD Arrests Data (Historic). This is a breakdown of every arrest carry out in NYC by the NYPD going back to 2006 through the end of the previous calendar year.
The dataset contains information about arrests made in NYC from 2006 to 2022, including the date and location of the arrest, the crime committed, and demographic information about the perpetrator.
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
nypd_arrests <- read.csv("https://raw.githubusercontent.com/omocharly/DATA698_Research-Project/refs/heads/main/mydata/NYPD%20Arrest%20Data%202019%20-2023.csv")
```
...
### Arrest by year:
2023 has the highest number of arrest of the five year under observation.
...
* There is no regular pattern on the arrest by quaters
...
ggplot(grouped_boro, aes(x = reorder(year, -count), y = count, fill = ARREST_BORO)) + 
  ...
  scale_fill_brewer(palette="blue") + theme_minimal()
```
...
```{r}
library(treemap)
treemap(nypd_arrests, 
        index=c("OFNS_DESC","LAW_CAT_CD"),
        vSize="ARREST_PRECINCT", 
        type="index",
